...
- **Quiz bank — the system.** The goal is that the student *memorizes and perfects* each chapter, so every lecture gets a **bank of many quizzes** where (a) each quiz alone covers the whole chapter, and (b) the quizzes differ in angle, wording and format so re-takes never feel like reciting one memorized run. Repetition across quizzes is welcome; tricks and hard questions are welcome — the only test is "does practising this make the Übung and the Klausur easier?" This recipe is **fixed and applies to every lecture and every future subject**.
  ...
  **Anti-tells & difficulty — the #1 rule, non-negotiable.** A question must be *impossible to answer correctly without knowing the material*. The single most common failure is a question that gives itself away by the *shape* of the options instead of their content. Audit every MCQ with this test: **cover the answer key — could a clueless student still pick the right option from the options alone?** If yes, it's broken. Specifically:
  - **Equal length & shape.** All options sit within a few words of each other. The correct answer is *never* systematically the longest, the most detailed, the most hedged, or the most "textbook-worded" one — that length/specificity tell is exactly what we forbid. If the right answer needs more words, pad the distractors to match (or trim the correct to its core). **Measurable bar (so it's verifiable, not eyeballed):** within each MCQ, the correct option must not be the single longest by ≥18 characters, *and* not the single shortest by ≥18 characters — aim for all four within ~12 chars. The fix is symmetric: lengthen the short distractors into genuine confusable neighbours (preferred — keeps content rich) and/or shorten an over-stuffed correct answer to its essence. Two reliable shortening moves: drop parenthetical examples the distractors don't have, and replace a long enumeration with its first items + „…".
  ...
  **Process — run in order, every time:**
  ...
  6. **Sweep the whole bank for length tells programmatically — don't trust the eye over hundreds of questions.** Parse every `options:[…]` array (a string-aware brace/bracket matcher is needed because options contain `{`, `[`, German quotes and escaped chars — a naïve regex grabs the question/explanation objects too), and for each MCQ compare `len(options[correct])` against the others. Flag any where the correct option is the strict longest by ≥18 chars *or* the strict shortest by ≥18 chars. Rewrite the flagged options (lengthen short distractors and/or trim the correct per the measurable bar), then re-run until the count is **0** for every lesson. Also confirm `correct` is spread roughly evenly across positions 0–3 across the bank (a position skew is its own tell). These checks are deterministic; rerun them after any later edit to the bank.
  7. **Verify:** `npx tsc --noEmit`, then open the chapter → Quiz in the browser, run a quiz end to end, confirm the picker lists every quiz **with its past-attempts history** (count, best %), answers + explanations show, **0 console errors**.

  **Definition of done — all must be true:** **exhaustive coverage** (every substantive fact in the lecture MD is tested somewhere in the bank — no gaps); many quizzes from the themed menu; ≥1 complete-run quiz that covers the chapter single-handed; every computable Übung skill has worked questions; every question grounded and carrying a short explanation; **no `easy` questions**; **anti-tells respected and the programmatic length-tell sweep returns 0 for the lesson (correct neither longest nor shortest by ≥18 chars), correct position spread evenly**; same German-quote-in-TS rule as explanations (never an ASCII `"` closing a German quote — use „ " or none); wired with `quizBankId`; renders with 0 console errors.
