# CySec2026_Exercise_02_Solutions
...
### VORLESUNG 21 14 17 11 4 18 20 13 6
`SECUNIDUE` 18 4 2 20 13 8 3 20 4

### Nachricht + Key 39 18 19 31 17 26 23 33 10
Z 26 13 18 19 5 17 0 23 7 10

### Chiffrat N S T F R A X H K
## 2. Länge 5: 26 ∗ 26 ∗ 26 ∗ 26 ∗ 26 = 265
Für den Text der Länge n: 26n

## 3. • 4 Ghz ≈ 4 ∗ 106 Operationen
- Effektiv also 106 Schlüsselberechnungen per Kern (CPU)
- 2Kerne = 2 ∗ 106Operationen
- 261024 ≈ 8, 56 ∗ 101448 Schlüssel
- 8, 56 ∗ 101448 ÷ (2 ∗ 106) ≈ 4, 28 ∗ 101442 Sekunden ≈ 1, 35 ∗ 101435 Jahre
## 4. Die doppelte Verschlüsselung hat keinen Nutzen, weil sich die Verschlüsselung mit Key k3 aus der
Summe von k1 + k2 darstellen lässt!

e(x) = (Buchstabe + k)mod26

2-fach verschlüsselts Vernam bedeutet: (Buchstabe + k1) mod 26 + (Buchstabe + k2) mod 26

Dies ist jedoch gleichwertig zu: (Buchstabe + (k1 + k2)) mod26, wobei (k1+k2), k3 ist
...
## 1. Es findet eine XOR-Addition des Klartexts mit dem Schlüsselstrom statt. Daher kann der Angreifer
beliebige Bitstrings auf das Chiffrat addieren, die sich direkt auf den Klartext auswirken.

## 2. Intention: Den Wert, welcher die Anzahl an Bitcoins darstellt direkt anpassen. Der Wert muss auf
25 oder höher gesetzt werden um 106 Euro in `BTC` (Bitcoin) zu erhalten. (106 ÷ 40 000 = 25) Dies

soll durch eine `XOR` Operation geschehen:
...
Es muss 11000 `XOR` addiert werden um zum Millionär zu werden.

## 3. Intention:
- dem Admin liegt das Chiffrat vor
- der Admin kennt die Position des ASCII Wertes m im Chiffrat
- das m soll zu p geändert werden
- es soll ein Wert angegeben werden, welcher dekodiert p ergibt Sie können einen beliebigen
Schlüsselstrom annehmen bzw. ein beliebiges Chiffrat, das dem Admin bekannt ist. Hier neh-

men wir an das Chiffrat für m ist: 100 000 Zunächst ermitteln wir den Schlüsselstrom s, da

wir Klartext und Chiffrat sowie die `XOR` Operation kennen:

m in `ASCII` 1 1 0 1 1 0 1

s = 0 1 0 1 1 0 1

y = 1 0 0 0 0 0 0

### Nun ermitteln wir das Chiffrat, welches p darstellt:
p in `ASCII` 1 1 1 0 0 0 0

s = 0 1 0 1 1 0 1

y = 1 0 1 1 1 0 1

Der Admin muss 1011101 in die Datenbank eintragen um das Premium-Abo nutzen zu kön-
...
### Aufgrund der Eigenschaft von XOR und
(
...
ist es möglich zuerst die Differenz zwischen m und p zu berechnen und dann das Chiffrat

durch `XOR` Addition mit dem Schlüsselstrom zu bestimmen.
1 `ASCII` Table
...
## 1. Ursprünglich 128 bit vorgesehen, aber auf drängen der NSA auf 56 bit reduziert.
## 2. • Konfusion: Eine Operation, um die Beziehung zwischen Schlüssel und Chiffrat zu verschlei-
ern. Bsp: Substitutionstabellen.

- Diffusion: Eine Operation, um den Einfluss eines Klartextsymbols auf zahlreiche Chiffrat-
symbole zu streuen. Bsp: Bitpermutation.

## 3. Siehe Vorlesungsfolien
...
- S1(000000) ⊕ S1(000001) = 1110̸ = S1(000000 ⊕ 000001) = 0000
- S1(111111) ⊕ S1(100000) = 1001̸ = S1(111111 ⊕ 100000) = 1000
- S1(101010) ⊕ S1(010101) = 1010̸ = S1(101010 ⊕ 010101) = 1101
